...
if (!(isset($_SESSION["Usuario"])&&$_SESSION["Usuario"])||!in_array($_SESSION["Usuario"]->getRol(), ["seleccionador", "administrador"]) || 
    ($DBmanager->ValorDeAtributoDeTablaExisteEnBD($_POST["NombreNuevo"], "nombre", "equipo") && $_POST["Nombre"] != $_POST["NombreNuevo"])) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['Imagen']) && $_FILES['Imagen']['type'] == 'image/png') {
    ...
    $directorioDestino = '../Assets/imagenesEquipos';
    ...
    if (move_uploaded_file($archivo['tmp_name'], $rutaDestino)) {
        ...
        foreach ($arrayEquipos as $equipoA) {
            if ($equipoA->nombre == $_POST["Nombre"]) {
                $equipo = $equipoA;
                break;
            }
        }
...
if ($nuevoNombre) { // Solo actualizar si el nuevo nombre no es null
    $DBmanager->UpdateEquipo($_POST["Nombre"], $_POST["NombreNuevo"], $nuevoNombre);
} else {
    echo "No se pudo generar un nuevo nombre de imagen.";
}

header("Location: ../Presentacion/listaEquipos.php");
exit;
?>
